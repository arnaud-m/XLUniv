...
#'
#' The tsv file contains a data frame gener#' Create a tsv file and a xlsx workbook used for the registration process of a programming contest based on PC^2.
#'
#' @param teamfile a csv file that contains the team information
#' @param filename a character prefix for the generated files
#' @param teamcapa the capacity of a team
...
}ated with \code{GenerateAccountPC2} with additional columns that give the members of the teams.
#'
#' The xlsx file contains three sheets:
#' \enumerate {
#' \item a human-readable version of the tsv file
...
CreatePC2AccountWorkbook <- function(teamfile, filename = "pc2account", nteams = 100, njudges = 4, teamcapa = NA) {
  ...
  write.table(tsvfile, file = paste(filename, ".tsv")) # Voir la destination

  # The xslx file
  wb <- XLConnect::loadWorkbook(paste(filename, ".xlsx"), create = TRUE) # voir la destination
  ...
  # Compact table sort by student sheet
  student1 <- is.na(tsvfile$Member1) == FALSE
  student2 <- is.na(tsvfile$Member2) == FALSE
  cstable <- data.frame(
               student = c(tsvfile$Member1[student1], tsvfile$Member2[student2]),
               login = c( paste(tsvfile$account[student1]), paste(tsvfile$account[student2]) ),
               password = c( paste(tsvfile$password[student1]), paste(tsvfile$password[student2]) )
