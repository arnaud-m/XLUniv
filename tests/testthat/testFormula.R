...
test_that("All Formulas work", {
  ...
  expect_equal(c("COUNTA(P:P)","AVERAGE(P:P)","STDEV(P:P)","COUNTIF(P:P, \">=10\")","COUNTIF(P:P, \"<=5\")","COUNTIF(P:P, \"<=1\")") , GetStatFormula(n = 0, m = 16,nmin=0)) # toute les lignes(doit marcher)
  expect_equal(c("COUNTA(P4:P4)","AVERAGE(P4:P4)","STDEV(P4:P4)","COUNTIF(P4:P4, \">=10\")","COUNTIF(P4:P4, \"<=5\")","COUNTIF(P4:P4, \"<=1\")") , GetStatFormula(n = 4, m = 16,nmin= 4 )) # meme ligne (doit marcher)
